...
void Touch_Init(void){
  ...
  SYSCTL_RCGC2_R |= SYSCTL_RCGC2_GPIOA;  // Activate PORTA GPIO clock
  wait++;
  wait++;
  
  GPIO_PORTA_DIR_R |= 0x0C;   // Configure PA2/PA3 for GPIO digital output
  
  
  SYSCTL_RCGC2_R |= SYSCTL_RCGC2_GPIOE;  // Activate PORTE GPIO clock
  wait++;
  wait++;
  
  GPIO_PORTE_DIR_R |= 0x30;   // Configure PE4/PE5 for GPIO digital output
